...
SELECT e.id, e.name, h.id FROM equipment e RIGHT OUTER JOIN equipment_suport_history h ON e.id=h.Equipment_id WHERE h.id=null;
SELECT e.id, e.name FROM equipment e LEFT JOIN equipment_suport_history h ON e.id=h.Equipment_id;
